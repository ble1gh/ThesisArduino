...
spine_tip_modelspace = [0,0,75]';

%coil to aurora transform, this is just using the first coil point, could
%filter to use more accurate
q = table2array(reg_tab(1,4:7));
t = table2array(reg_tab(1,8:10));
TF_coil_to_aurora = eye(4);
TF_coil_to_aurora(1:3,1:3) = quat2matrix(q);
TF_coil_to_aurora(1:3,4) = t;
...
for coil_idx = 1:size(rawcoil_tab,1)
    q = table2array(rawcoil_tab(coil_idx,4:7));
    t = table2array(rawcoil_tab(coil_idx,8:10));
...
%plot
figure(1)
plot3(spinedata_in_modelspace(1,:),spinedata_in_modelspace(2,:),spinedata_in_modelspace(3,:)),
axis equal
xlabel('x')
ylabel('y')
zlabel('z')