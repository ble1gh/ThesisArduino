...
fit_coeffs_tab = readtable("x_coeffs_start.csv");
fit_coeffs_xz = table2array(fit_coeffs_tab);
fit_coeffs_xz = fit_coeffs_xz(2:6,10);
xfit_coeffs = zeros(5,iterations+1,res_theta);
z_coeffs_tab = readtable("z_coeffs_start.csv");
z_coeffs = table2array(z_coeffs_tab);
z_coeffs = z_coeffs(2:6,10);
...
for i = 1:res_theta
    ...
    figure(3)
    plot(xfit_motor1,r(1,:,k,i),dlplanefit)
    grid on;
    xlabel('r')
    ylabel('dlplane')
    ...
    x_avg_coeffs(2:6,:,i) = xfit_coeffs(:,:,i);
    z_avg_coeffs(2:6,:,i) = z_coeffs(:,:,i);
end

writematrix(x_avg_coeffs,'x_all_coeffs_Q1.csv')
writematrix(z_avg_coeffs,'z_all_coeffs_Q1.csv')
writematrix(points_record,'points_record_Q1.csv')
