...
for i = 1:res_theta
    ...
    % x_req = r_req*cos(theta(i));
    % y_req = r_req*sin(theta(i));
    % z_pred = z_coeffs(1)*r_req.^4 + z_coeffs(2)*r_req.^3 + z_coeffs(3)*r_req.^2 + z_coeffs(4)*r_req.^1 + z_coeffs(5);
    % 
    % req = [x_req; y_req; z_pred];
