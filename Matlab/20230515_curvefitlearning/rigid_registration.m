...
c1 = [50*cos(3*pi/4) 50*sin(3*pi/4) -5-1.57]';
c2 = [50*cos(5*pi/4) 50*sin(5*pi/4) -5-1.57]';
c3 = [50*cos(7*pi/4) 50*sin(7*pi/4) -5-1.57]';
c4 = [50*cos(pi/4)+10*cos(3*pi/4) 50*sin(pi/4)+10*sin(3*pi/4) -5-1.57]';

regpt_spine = [c1 c2 c3 c4];


% load tip file
penprobe_tip = load('tipcal2.tip');

% load registration points from aurora and apply tip compensation
reg_tab = readtable('20230516_reg_001.csv','Delimiter',',');
...
straight_tab = readtable('20230516_reg_001.csv','Delimiter',',');
qcoil = table2array(straight_tab(2,4:7));
tcoil = table2array(straight_tab(2,8:10));
% qcoil = regpt_aurora(2,4:7);
% tcoil = regpt_aurora(2,8:10);
...
rawcoil_tab = readtable('fitted_output.csv','Delimiter',',');
...
writematrix(spinedata_in_modelspace,'transformed_output_0516_xz.csv')
